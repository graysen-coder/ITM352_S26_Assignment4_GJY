...
# This function compares the current session score against every score saved in the leaderboard
# JSON file and returns a dict with the percentile beaten, mean score, and z-score so the
# end screen can show the player how they did relative to others in infinite mode
# The z-score uses the sample standard deviation formula and includes a plain-language note
# field for edge cases like an empty leaderboard, only one saved score, or all tied scores
...
# This function generates a single-panel run chart showing damage dealt vs damage taken
# across all waves using matplotlib and saves it as a PNG to the static folder
...
# For infinite mode it also calls compare_score_to_leaderboard() to generate percentile and z-score
# stats, and calls _build_run_chart() to generate the summary chart image
@app.route("/end")
def end():
...
# For defenders it uses scraped DLNR facts if available, falling back to the facts in defenders.json
# For invaders it loads all cards from the invaders JSON and passes description and impact points
# to the template alongside the standard card metadata
